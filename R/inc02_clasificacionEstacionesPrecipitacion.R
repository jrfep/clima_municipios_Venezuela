...
adm2 <- readRDS(sprintf("%s/VEN_adm2.rds",vector.dir))
...
load(sprintf("%s/climaBolívar.rda",data.dir))
...
svg(file=sprintf("%s/ClasificacionEstaciones_EstadoBolivar.svg",
        out.dir),width=8,height=8)

par(mar=c(0,0,0,0),fig=c(0,1,0,1),new=F)
plot(subset(adm2,NAME_1 %in% "Bolívar"),border="pink",col="aliceblue",lwd=1.5)

if (require(png) & exists("logo"))
    rasterImage(logo, -67.7, 2.15, -66., 3.85)

symbols(xys[,1],xys[,2],circles=sqrt(rowSums(mtz)),inches=.10,
        bg=clrs[grp],fg=1,add=T)
title("Precipitación promedio mensual [mm] por estaciones climáticas",line=-2)
title("Estado Bolívar",line=-3)

par(fig=c(.06,.26,.75,.9),new=TRUE,las=2,cex.axis=.8)
barplot(apply(mtz[grp==1,],2,median),col=clrs[1],ylim=c(0,520),main="Promedio Grupo 1",xpd=NA,cex.main=.9)
par(fig=c(.15,.35,.27,.42),new=TRUE,las=2,cex.axis=.8)
barplot(apply(mtz[grp==2,],2,median),col=clrs[2],ylim=c(0,520),main="Promedio Grupo 2",xpd=NA,cex.main=.9)
par(fig=c(.7,.9,.08,.23),new=TRUE,las=2,cex.axis=.8)
barplot(apply(mtz[grp==3,],2,median),col=clrs[3],ylim=c(0,520),main="Promedio Grupo 3",xpd=NA,cex.main=.9)
